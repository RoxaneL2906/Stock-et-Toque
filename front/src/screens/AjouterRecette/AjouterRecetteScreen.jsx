...
function AjouterRecetteScreen({ onNaviguer }) {
  const [photo, setPhoto] = useState(null);
  const [apercuPhoto, setApercuPhoto] = useState(null);
  const [titre, setTitre] = useState('');
  const [description, setDescription] = useState('');
  const [ingredients, setIngredients] = useState([{ nom: '', quantite: '', unite: '', alternative: '' }]);
  ...
  const soumettre = async (brouillon) => {
    setErreur('');
    setSucces('');
    setChargement(true);

    try {
      const resultat = await creerRecette({
        titre,
      ...
      if (photo) {
        await uploaderPhotoRecette(resultat.id, photo);
      }

      setSucces(brouillon ? 'Recette enregistrée en brouillon.' : 'Recette publiée avec succès.');
      setTimeout(() => onNaviguer('mesRecettes'), 1200);
    } catch (err) {
  ...
  return (
    <div className="ecran-complet">
      ...
      <div className="ecran-contenu">
        ...
        <label className="ajout-recette-photo-zone">
          ...
          {apercuPhoto ? (
            ...
            <div className="ajout-recette-photo-contenu">
              ...
              <span className="ajout-recette-photo-texte">Ajouter une photo</span>
        ...
        <PrimaryButton
          texte={chargement ? 'Publication...' : 'Publier'}
          onClick={() => soumettre(false)}
        />

        <button
          type="button"
          className="ajout-recette-lien-brouillon"
...
export default AjouterRecetteScreen;